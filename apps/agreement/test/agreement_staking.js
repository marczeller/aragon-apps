...
contract('Agreement', ([_, someone, signer]) => {
  ...
  before('deploy collateral token', async () => {
  ...
  describe('stake', () => {
    ...
    const itStakesCollateralProperly = amount => {
      context('when the signer has approved the requested amount', () => {
        beforeEach('approve tokens', async () => {
          await agreement.approve({ amount, from: signer, to: agreement.staking.address })
        })
        ...
        it('does not affect the locked balance of the signer', async () => {
          const { locked: previousLockedBalance } = await agreement.getSigner(signer)

          await agreement.stake({ amount, signer, approve })

          const { locked: currentLockedBalance } = await agreement.getSigner(signer)
        ...
        it('transfers the staked tokens to the contract', async () => {
          ...
          const previousAgreementBalance = await collateralToken.balanceOf(agreement.staking.address)
          ...
          const currentAgreementBalance = await collateralToken.balanceOf(agreement.staking.address)
      ...
      context('when the signer has not approved the requested amount', () => {
        it('reverts', async () => {
          await assertRevert(agreement.stake({ amount, signer, approve }), ERRORS.ERROR_STK_COLLATERAL_TOKEN_TRANSFER_FAILED)
        })
      })
    }
    ...
    context('when the amount is below the collateral amount', () => {
      ...
      it('reverts', async () => {
        await assertRevert(agreement.stake({ amount, signer, approve }), ERRORS.ERROR_STK_AVAILABLE_BALANCE_BELOW_COLLATERAL)
      })
    })
  ...
  describe('stakeFor', () => {
    ...
    const itStakesCollateralProperly = amount => {
      context('when the signer has approved the requested amount', () => {
        beforeEach('approve tokens', async () => {
          await agreement.approve({ amount, from, to: agreement.staking.address })
        })
      ...
      context('when the signer has approved the requested amount', () => {
        it('reverts', async () => {
          await assertRevert(agreement.stake({ signer, amount, from, approve }), ERRORS.ERROR_STK_COLLATERAL_TOKEN_TRANSFER_FAILED)
        })
      })
    }
    ...
    context('when the amount is below the collateral amount', () => {
      ...
      it('reverts', async () => {
        await assertRevert(agreement.stake({ signer, amount, from, approve }), ERRORS.ERROR_STK_AVAILABLE_BALANCE_BELOW_COLLATERAL)
      })
    })
  ...
  describe('approveAndCall', () => {
    ...
    const itStakesCollateralProperly = amount => {
      ...
      it('emits an event', async () => {
        ...
        const logs = decodeEventsOfType(receipt, agreement.staking.abi, EVENTS.BALANCE_STAKED)
    ...
    context('when the amount is below the collateral amount', () => {
      ...
      it('reverts', async () => {
        await assertRevert(agreement.approveAndCall({ amount, from, mint: false }), ERRORS.ERROR_STK_AVAILABLE_BALANCE_BELOW_COLLATERAL)
      })
    })
  ...
  describe('unstake', () => {
    ...
    context('when the sender has some amount staked before', () => {
      ...
      context('when the requested amount greater than zero', () => {
        ...
        context('when the requested amount is lower than or equal to the actual available balance', () => {
          ...
          context('when the remaining amount is below the collateral amount', () => {
            ...
            it('reverts', async () => {
              await assertRevert(agreement.unstake({ signer, amount }), ERRORS.ERROR_STK_AVAILABLE_BALANCE_BELOW_COLLATERAL)
            })
          })
        ...
        context('when the requested amount is higher than the actual available balance', () => {
          ...
          it('reverts', async () => {
            await assertRevert(agreement.unstake({ signer, amount }), ERRORS.ERROR_STK_NOT_ENOUGH_AVAILABLE_STAKE)
          })
        })
      })
      ...
      context('when the requested amount is zero', () => {
        ...
        it('reverts', async () => {
          await assertRevert(agreement.unstake({ signer, amount }), ERRORS.ERROR_STK_INVALID_UNSTAKE_AMOUNT)
        })
      })
    })
