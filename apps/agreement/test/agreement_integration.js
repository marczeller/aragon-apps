...
contract('Agreement', ([_, challenger, holder0, holder1, holder2, holder3, holder4, holder5]) => {
  ...
  describe('integration', () => {
    ...
    it('computes available stake balances properly', async () => {
      ...
      const { available: holder1Available, locked: holder1Locked } = await agreement.getSigner(holder1)
      ...
      const { available: holder2Available, locked: holder2Locked } = await agreement.getSigner(holder2)
      ...
      const { available: holder3Available, locked: holder3Locked } = await agreement.getSigner(holder3)
      ...
      const { available: holder4Available, locked: holder4Locked  } = await agreement.getSigner(holder4)
      ...
      const { available: holder5Available, locked: holder5Locked } = await agreement.getSigner(holder5)
      ...
      const stakingBalance = await collateralToken.balanceOf(agreement.staking.address)
